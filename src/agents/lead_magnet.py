...
[필수 룰 — 어기면 즉시 재생성]
1. 길이 20자 이내 (40자 X)
2. **카테고리 가시성** — hook 안에 다음 중 **최소 1개 필수**:
   - 도구·브랜드명 (ChatGPT/Claude/Cursor/Notion AI/Zapier 등)
   - 카테고리 명사 (AI 툴/코딩/엑셀 자동화/SNS 마케팅 등)
   - 1인칭 주어 (내가/나만/제가)
   - 행동 객체 (앱 만들기/자소서 첨삭/발표자료 등)
...
[self-check — hook 작성 후 직접 답할 것]
- 이 hook을 처음 본 사람이 **1초 안에 "AI 카테고리 콘텐츠"임을 알 수 있는가**?
- 숫자가 있다면 **그 숫자가 무엇의 숫자인지** 같은 줄에서 보이는가?
- 카테고리 명사·도구명·1인칭 중 하나라도 있는가?
- 셋 다 NO면 hook 다시 작성. 통과할 때까지 출력 X.
