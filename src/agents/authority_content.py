...
{_BASE_CSS}
  .wrap {{
    ...
    justify-content:center; padding:104px 96px;
  }}
  .overlay {{ {overlay_css} }}
  .gold-top {{
...
🚫 **단일 주제 룰 — 4개 insight 모두 [주제]와 동일한 단일 카테고리 안에서만 작성.**
   - 시장 분석 주제면 → 4개 모두 시장 분석 (실거래·시세·공급·수요 등)
   - 인테리어 주제면 → 4개 모두 인테리어 (자재·조도·평면 등)
   - 카테고리 혼합(예: 시장 분석 2개 + 인테리어 2개)은 콘텐츠 정체성 붕괴 — 알고리즘 카테고리 학습 실패 + 팔로워 이탈
   - 의심되면 같은 카테고리 안에서 4번째 insight 못 채우면 3개로 제출하고 빈 자리에 "(생략)" 표기

✅ 이렇게 써:
- 확인된 데이터에서 통찰을 뽑아라 (수치가 없으면 "패턴", "흐름" 등 정성 표현)
- 문장: 간결하고 단호하게. 군더더기 없이. 전문가가 후배에게 말하듯.
- 팔로워가 "이 계정만 알고 있다"고 느끼게
- 4개 insight를 모두 작성한 후 self-check: "이 4개가 모두 같은 카테고리인가?" 다르면 다시 써라
...
{{
  ...
  "cta_text": "DM 매물 문의 유도 메인 카피 (20자 이내, 자산가 톤 — 예: '구체적 매물은 DM으로 직접', '오프마켓 매물 DM 문의')",
  "cta_sub": "보조 행동 1줄 (15자 이내, 저장 유도는 부차 — 예: '저장은 시장 분석 구독 가치')",
