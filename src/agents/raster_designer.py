...
## 권위·인물 호칭 (Hard, 2026-05-12 사용자 명시)
- ❌ "교수" 단독 사용 금지 → ✅ **"교수님"** 강제 (어그로 패턴 안에서도 유지)
- ❌ "선생" → ✅ "선생님" / ❌ "조교·강사" → ✅ "조교님·강사님"
- 권위·연장자·전문가 호칭은 일관 존댓말. 친근체와 존댓말 호칭 충돌 X.
- 예: "교수님이 알면 화낼 ~~" / "교수님 시각으로 첨삭받기" / "교수님이 빨간 펜 들기 전에"
- 이유: 라운드 7 "교수가 알면 화낼" 카피 후 사용자 톤 다정 요청. 직설·실용 톤이되 권위 호칭은 다정 존댓말 일관.
</headline_voice>
...
## 권위 호칭 (Hard, 2026-05-12 사용자 명시)
- ❌ "교수" 단독 → ✅ **"교수님"** 강제 (어그로 패턴 안에서도 유지)
- ❌ "선생/조교/강사" → ✅ "선생님/조교님/강사님"
- 권위·연장자 호칭은 일관 존댓말. visual prompt 안 한글 slot에서도 동일 적용.
- 사유: 라운드 7 "교수가 알면 화낼" 카피 후 사용자 톤 다정 요청.
</headline_voice>
</include>
<exclude>
...
- **권위 호칭은 일관 존댓말**: "교수" 단독 금지 → "교수님" / "선생" → "선생님" / "조교·강사" → "조교님·강사님". 어그로 패턴 안에서도 유지 ("교수님이 알면 화낼 ~~"). 직설 톤이되 권위 호칭은 다정한 존댓말 일관.

[금지]
- 카드뉴스 본문 그대로 복붙 금지 (카드뉴스는 미끼·요약, 가이드는 풀콘텐츠)
- 슬라이드 번호·시각 묘사 언급 금지 (가이드는 텍스트 only)
- 일반론·교과서 톤 금지 — 구체·체감·실수 위주
- **body 안에 영문 큰따옴표(") 사용 절대 금지** — JSON 파싱 깨짐. 인용은 한국어 「」 또는 작은따옴표 '...' 사용. 강조는 평문으로.
...
def _generate_dm_guide_sections(
    ...
    응답 파싱 강건성: sections가 str이면 JSON parse / max_tokens truncate 명시 에러 / 1회 retry.
    """
    import anthropic
...
5개 비결 각각 정확히 700~900자 한글 풀콘텐츠 JSON 출력."""
    ...
    def _call() -> list[dict]:
        ...
        if resp.stop_reason == "max_tokens":
            raise RuntimeError(f"dm_guide max_tokens truncated (stop_reason=max_tokens)")
        tool_use = [b for b in resp.content if getattr(b, "type", None) == "tool_use"]
        if not tool_use:
            raise RuntimeError(f"dm_guide tool_use 미사용. stop_reason={resp.stop_reason} content={resp.content[:2]}")
        raw = tool_use[0].input.get("sections")
        # 강건성: str이면 JSON parse 시도
        if isinstance(raw, str):
            print(f"[dm-guide] sections가 str ({len(raw)}자) — JSON parse 시도")
            raw = json.loads(raw)
        if not isinstance(raw, list):
            raise RuntimeError(f"sections 타입 비정상: {type(raw).__name__} (기대: list)")
        if len(raw) != 5:
            raise RuntimeError(f"sections 5개 필요, 받음 {len(raw)}")
        return raw

    print(f"[dm-guide] Sonnet 4.6 호출 — 5개 비결 × 800자 (max_tokens=12000)...")
    try:
        sections = _call()
    except RuntimeError as e:
        print(f"[dm-guide] 1차 실패: {e} — 재시도 1회")
        sections = _call()
