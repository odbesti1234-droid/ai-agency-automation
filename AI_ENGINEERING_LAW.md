...
## 📜 제11조 — 카드뉴스 디자인 최소 품질 기준 (Design Quality Floor)

> **이 기준 미달 카드는 Slack 전송·Instagram 발행 모두 금지한다.**
> 기준점: `C:\Users\Administrator\Documents\카드뉴스\extracted\export\slides\` 6장 (2026-04-19 유선우 지정)

### 11.1 디자인 불변 원칙 (절대 위반 금지)

```
1. 전 슬라이드 다크 배경 일관 유지
   ❌ 금지: 슬라이드마다 배경색 변경, 화이트 배경(#FAFAF8 등) 사용
   ✅ 필수: 브랜드 primary 다크 컬러로 전체 통일 (luminance < 0.35 강제)

2. 골드/엑센트 컬러 계층 구조
   ✅ 필수: 배경(다크) + 엑센트(골드/브랜드 세컨더리) + 텍스트(크림화이트) 3계층

3. 얇은 테두리 프레임 + 코너 브래킷
   ✅ 필수: inset 40px, 1.5px border, 4개 코너 L자 브래킷

4. 진행 도트 표시
   ❌ 금지: "1 / N" 텍스트 표시만
   ✅ 필수: ● ● ● ● 점 형태 (현재 슬라이드 활성 강조)

5. 대형 세리프 번호 (컨텐츠 슬라이드)
   ✅ 필수: 240px serif 금색 번호 (01, 02, 03...) + 수평 구분선

6. thin-line SVG 아이콘 (컨텐츠 슬라이드)
   ✅ 필수: 슬라이드 위치별 thin-line 아이콘 (검색/차트/저울/북마크/캘린더 순환)

7. 브랜드 Footer 전 슬라이드
   ✅ 필수: 모든 슬라이드 하단에 브랜드명 표기

8. 타이포그래피 폰트 기준
   ✅ 필수: Playfair Display (이탤릭·번호) + Noto Serif KR (헤드라인) + Noto Sans KR (본문)
```

### 11.2 슬라이드 구조 표준

```
슬라이드 1 (커버):   배경|프레임|코너|시리즈 라벨|메인 훅|스와이프 힌트|푸터
슬라이드 2~N (컨텐츠): 배경|프레임|코너|위치 레이블|대형 번호+아이콘|구분선|헤드라인|본문|푸터
슬라이드 N+1 (CTA):  배경|프레임|코너|SAVE·FOLLOW 헤더|메인 메시지|브랜드 핸들|북마크 버튼|푸터
```

### 11.3 품질 게이트 (card_designer 실행 전 체크)

designer 에이전트는 카드 생성 후 아래를 자동 검증해야 한다:
- [ ] 모든 슬라이드 배경 luminance < 0.35 (다크 확인)
- [ ] PNG 파일 크기 > 50KB (빈 슬라이드 방지)
- [ ] 슬라이드 수 ≥ 3 (커버 + 최소 1 컨텐츠 + CTA)

검증 실패 시 → `status: design_failed` 기록, Slack 오류 알림, Instagram 발행 차단.

### 11.4 예외 없음

```
이 조항은 아래 제10조 2항(불변 조항)에 추가된다.
어떤 클라이언트, 어떤 일정, 어떤 이유로도 이 기준 미달 카드는 발행하지 않는다.
"빠르게 내보내는 것"이 목적일 때도 이 기준이 우선한다.
```
...
| 2026-04-19 | v1.1 | 제11조 카드뉴스 디자인 최소 품질 기준 추가 | 유선우 지정·Claude 집행 |
